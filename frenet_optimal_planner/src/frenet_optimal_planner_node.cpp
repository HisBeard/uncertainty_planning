...
FrenetOptimalPlannerNode::FrenetOptimalPlannerNode() : tf_listener(tf_buffer)
{
  ...
  // vehicle_cmd_pub = nh.advertise<geometry_msgs::Twist>(vehicle_cmd_topic, 1);
  // vehicle_cmd_pub = nh.advertise<carla_msgs::CarlaEgoVehicleControl>(vehicle_cmd_topic, 1);
  vehicle_cmd_pub = nh.advertise<ackermann_msgs::AckermannDrive>(vehicle_cmd_topic, 1);
  obstacles_pub = nh.advertise<autoware_msgs::DetectedObjectArray>("local_planner/objects", 1);
  test_pub = nh.advertise<std_msgs::Float64>("yaw_diff", 1);
...
void FrenetOptimalPlannerNode::concatPath(const fop::FrenetPath& next_traj, const int traj_max_size, const int traj_min_size, const double wp_max_seperation, const double wp_min_seperation)
{
  ...
  if (curr_trajectory_.x.size() <= traj_min_size)
  {
    ...
    for (size_t i = 0; i < diff; i++)
    {
      ...
      if (wp_seperation >= wp_max_seperation)
      {
        ROS_WARN("Local Planner: waypoint out of bound, rejected 1 %f", wp_seperation);
        regenerate_flag_ = true;
        break;
      }

      if (wp_seperation <= wp_min_seperation)
      {
        ROS_WARN("Local Planner: waypoint out of bound, rejected 2 %f", wp_seperation);
        regenerate_flag_ = true;
        break;
      }
...
bool FrenetOptimalPlannerNode::calculateControlOutput(const int next_wp_id, const fop::VehicleState& frontaxle_state)
{
  ...
  {
    ROS_ERROR("Local Planner: Output Path Too Short! No output steering angle %d %d", curr_trajectory_.x.size(), wp_id + 2);
  ...
  else
  {
    ...
    // use prediction velocity diretly
    acceleration_ = curr_trajectory_.v[wp_id];
    ...
    msg.data = fop::rad2deg(delta_yaw);
...
void FrenetOptimalPlannerNode::publishVehicleCmd(const double accel, const double angle)
{
  ...
  vehicle_cmd.steering_angle = angle;
  vehicle_cmd.steering_angle_velocity = 0;
  vehicle_cmd.speed = accel;
